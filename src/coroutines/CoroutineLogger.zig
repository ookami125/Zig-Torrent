...
pub const CoroutineLogger = struct {
	...
	pub fn process(self: *@This(), ctx: *CoroutineContext) !bool {
		...
		if(self.timer < std.time.milliTimestamp()) {
			var timestamp: u64 = @intCast(std.time.milliTimestamp() - self.start);
			var hours = @divTrunc(timestamp % std.time.ms_per_day, std.time.ms_per_hour);
			var mins = @divTrunc(timestamp % std.time.ms_per_hour, std.time.ms_per_min);
			var secs = @divTrunc(timestamp % std.time.ms_per_min, std.time.ms_per_s);
